...
onMounted(() => {
  ...
  toggleButton.addEventListener('click', function() {
    ...
    if(localStorage.getItem('color-theme')) {
      if(localStorage.getItem('color-theme') === 'light') toggleToDark()
      else toggleToLight()
    } else {
      if(document.documentElement.classList.contains('dark')) toggleToLight()
      else toggleToDark()
    }
  }) 
}) 
...
<style scoped>
  .themeToggle {
    @apply absolute inset-5 w-12 h-12 flex justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-sm p-2.5 
  }
</style>